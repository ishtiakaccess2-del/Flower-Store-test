...
// ==========================================
// ACTION EVENTS BINDINGS FOR RENDERED PRODUCTS
// ==========================================
function attachProductCardActionEvents() {
    ...
    grid.querySelectorAll(".quick-view-btn").forEach(btn => {
        btn.addEventListener("click", (e) => {
            e.stopPropagation();
            openQuickViewModal(btn.getAttribute("data-id"));
        });
    });
    ...
    grid.querySelectorAll(".direct-wa-order-btn").forEach(btn => {
        btn.addEventListener("click", (e) => {
            e.stopPropagation();
            const id = btn.getAttribute("data-id");
            const prod = PRODUCTS.find(p => p.id === id);
            if (prod) {
                openSingleProductCheckoutModal(prod, 1);
            }
        });
    });
}
...
function attachSliderActionEvents() {
    ...
    slider.querySelectorAll(".quick-view-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            openQuickViewModal(btn.getAttribute("data-id"));
        });
    });

    slider.querySelectorAll(".add-to-cart-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            addToCart(btn.getAttribute("data-id"), 1);
        });
    });
...
function updateCartUI() {
    ...
    if (container) {
        if (cart.length === 0) {
            container.innerHTML = `
                <div class="h-full flex flex-col items-center justify-center text-center text-on-variant/50 py-12">
                    <span class="material-symbols-outlined text-5xl">shopping_cart_off</span>
                    <p class="mt-2 text-xs font-semibold">Your selection basket is empty.</p>
                </div>
            `;
        } else {
            container.innerHTML = cart.map(item => `
                <div class="flex items-center gap-4 bg-surface-low p-3 rounded-lg border border-white/5">
                    <img src="${item.image}" alt="${item.name}" class="w-12 h-12 object-cover rounded" />
                    <div class="flex-1">
                        <h4 class="text-xs font-bold text-on-surface truncate">${item.name}</h4>
                        <p class="text-primary text-[11px] font-black mt-0.5">৳${item.price.toFixed(2)}</p>
                        <div class="flex items-center gap-2 mt-2">
                            <button class="cart-minus-btn bg-surface hover:bg-primary/20 text-on-surface hover:text-primary h-5 w-5 rounded flex items-center justify-center border border-white/10" data-id="${item.id}">-</button>
                            <span class="text-xs font-semibold px-1">${item.quantity}</span>
                            <button class="cart-plus-btn bg-surface hover:bg-primary/20 text-on-surface hover:text-primary h-5 w-5 rounded flex items-center justify-center border border-white/10" data-id="${item.id}">+</button>
                        </div>
                    </div>
                    <button class="cart-del-btn text-on-variant/50 hover:text-primary transition" data-id="${item.id}">
            ...
            container.querySelectorAll(".cart-minus-btn").forEach(btn => {
                btn.addEventListener("click", () => updateCartQuantity(btn.getAttribute("data-id"), "decrease"));
            });
            container.querySelectorAll(".cart-plus-btn").forEach(btn => {
                btn.addEventListener("click", () => updateCartQuantity(btn.getAttribute("data-id"), "increase"));
            });
    ...
    if (subtotalText) {
        subtotalText.textContent = `৳${calculateCartSubtotal().toFixed(2)}`;
    }
}
...
function updateUserDisplayState(user) {
    ...
    if (nameDisplay) {
        if (user.email === ADMIN_EMAIL) {
            nameDisplay.textContent = "Admin Panel";
        } else {
            nameDisplay.textContent = user.displayName || "My Account";
        }
    }

    if (mobileUserStatus) {
        mobileUserStatus.textContent = `Logged in as: ${user.displayName || user.email}`;
    }
...
// ==========================================
// TRENDING SLIDER DRAG AND BUTTON SCROLL SUPPORT
// ==========================================
function initTrendingSliderDragSupport() {
    const container = document.getElementById("trending-slider-container");
    if (!container) return;
...
// ==========================================
// MODALS SYSTEM WITH POINTER EVENT TOGGLES
// ==========================================
...
function showToast(text, type = "success") {
    ...
    if (!container) {
        ...
        container.className = "fixed bottom-6 left-6 z-50 flex flex-col gap-3 pointer-events-none";
